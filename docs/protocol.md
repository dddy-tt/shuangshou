...
报警/消警优先于普通遥测。USART3 仍为 9600、8N1；`FLEX` 和已有最近有效姿态的 `IMU` 每 `250 ms` 发送，`CARE`、`PPG`、`JY`、`ACC`、`ALARM_STATE` 每 `1000 ms` 发送，`BRINGUP` 每 `5000 ms` 发送。此节流将典型文本负载降至约 `600~650 B/s`，为 9600、8N1 理论约 `960 B/s` 留出队列余量；实际余量仍取决于字段长度和活动报警重发。发送 API 每次提交的完整字节单元不可被另一优先级插入；单元内部可分成多个不超过 64 字节的 HAL 传输，只有单元完成后才重新择优。当前固件的文本生产者均一次提交完整 `\r\n` 行；`BT_SendRaw` 仍支持任意原始字节，调用者若发送协议文本也应一次提交完整行。
