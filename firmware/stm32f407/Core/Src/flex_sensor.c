...
static uint8_t flex_finger_enabled(uint8_t hand, uint8_t finger)
{
    static const uint8_t left_enabled[FINGER_NUM] = {
        FLEX_LEFT_L1_ENABLED,
        FLEX_LEFT_L2_ENABLED,
        FLEX_LEFT_L3_ENABLED,
        FLEX_LEFT_L4_ENABLED,
        FLEX_LEFT_L5_ENABLED
    };

    if (finger >= FINGER_NUM) return 0U;
    return hand ? left_enabled[finger] : 1U;
}
...
void Flex_Update(void)
{
    ...
    for (uint8_t h = 0; h < 2; h++) {
        Flex_Finger_t *hand = h ? Hand_Left : Hand_Right;
        /*
         * Current glove harness mapping:
         *   ADC1 (PC3, PA1..PA4) -> physical left hand
         *   ADC2 (PA5..PA7, PB0..PB1) -> physical right hand
         *
         * Keep the public API unchanged: hand=0 is right and hand=1 is left.
         * Only the ADC source is exchanged here, at the hardware boundary.
         */
        volatile uint16_t *buf_full = h ? adc1_buf : adc2_buf;
        uint8_t stable_half = h ? half_adc1 : half_adc2;
        /* The source buffer above is ADC1 for the left hand and ADC2 for
         * the right hand, so its ready bit must follow the same mapping. */
        uint8_t has_fresh_snapshot = h ? ((ready & 0x01U) != 0U)
                                        : ((ready & 0x02U) != 0U);
        ...
        for (uint8_t i = 0; i < FINGER_NUM; i++) {
            if (!flex_finger_enabled(h, i)) {
                /* Keep unused ADC channels out of calibration and telemetry. */
                hand[i].current_raw = 0U;
                hand[i].adc_min = 4095U;
                hand[i].adc_max = 0U;
                hand[i].pos_percent = 0U;
                continue;
            }
