...
function buildFingerRows(
  ...
  normalizedValues: Array<number | null> | null
) {
...
export const SensorMonitorPanel: React.FC<Props> = ({
  flex,
  imu,
  onZero,
  onFull,
  ...
  const fingerRows = [
    ...buildFingerRows("左手", flex.leftFingers, flex.normalizedLeftFingers),
    ...buildFingerRows("右手", flex.rightFingers, flex.normalizedRightFingers)
  ];
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            展示十指 FLEX 数值、归一化结果和 IMU 姿态，支持简单置零与握满校准。
          </p>
        </div>
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {fingerRows.map((finger) => (
              <div key={finger.id} className="rounded-2xl border border-white/10 bg-slate-950/25 p-4">
                ...
                <p className="mt-2 text-sm text-slate-400">
                  归一化 <span className="font-bold text-cyan-100">{formatNumber(finger.normalized, 2)}</span>
                </p>
              </div>
            ))}
          </div>
