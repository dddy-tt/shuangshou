...
import { AiFeedbackSource } from "../types";
...
export const FeedbackCard: React.FC<Props> = ({ feedback, isLoading, source }) => (
    ...
    <div className="mt-5 flex min-h-[220px] items-center rounded-[24px] border border-fuchsia-300/10 bg-gradient-to-br from-fuchsia-300/10 via-transparent to-sky-300/10 p-5">
      {isLoading ? (
        <div className="mx-auto text-center">
          ...
          <p className="mt-4 font-bold text-fuchsia-100">正在生成训练反馈...</p>
        </div>
      ) : feedback ? (
        ...
        <div className="w-full text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">完成一次训练采集后，这里会显示 bridge 或本地回退生成的训练反馈。</p>
        </div>
      )}
    </div>
  </section>
