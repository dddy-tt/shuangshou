import { Archive, Shuffle, Trash2 } from "lucide-react";
...
export const CustomGestureManager: React.FC<Props> = ({
  gestureName,
  actionText,
  category,
  items,
  ...
  <section className="panel">
    <div className="flex items-center gap-3 border-b border-white/10 pb-3">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">保存当前十指和 IMU 快照，命中后可触发翻译、控制或训练提示。</p>
      </div>
    </div>
    ...
    <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-[0.95fr_1.05fr]">
      <div className="rounded-2xl border border-white/10 bg-slate-950/25 p-4">
        <div className="grid grid-cols-1 gap-4">
          ...
          <button className="tab-button tab-button-idle w-full" disabled={busy} onClick={onPickRandomTraining}>
            <Shuffle size={16} className="mr-2 inline-block" />
            随机选择训练手势
          </button>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-fuchsia-300/15 bg-fuchsia-300/10 p-4 text-sm text-slate-200">
          <p className="font-bold text-fuchsia-100">当前训练提示</p>
