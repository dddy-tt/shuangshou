const { classifyFinger, compareGesture, createStableMatcher } = require('../../services/gesture-matcher');
const { createGestureStore } = require('../../services/gesture-store');
const { createTtsService } = require('../../services/tts');
const { formatMetric, nextPose, roundForUi } = require('../../utils/realtime-view');
const labels = ['拇指', '食指', '中指', '无名指', '小指'];
function toFinger(value, index, enabled) { if (!enabled) return { label: labels[index], value: 0, state: '已关闭', tone: 'idle', disabled: true }; const state = classifyFinger(value); const tone = state === 'straight' ? 'success' : state === 'half' ? 'warning' : state === 'full' ? 'danger' : 'idle'; return { label: labels[index], value: Number.isFinite(value) ? roundForUi(value) : 0, state: state === 'straight' ? '平直' : state === 'half' ? '半弯' : state === 'full' ? '全弯' : '等待', tone, disabled: false }; }
function poseKey(pose) { return `${formatMetric(pose.roll)}|${formatMetric(pose.pitch)}|${formatMetric(pose.yaw)}`; }
function ttsStatus(result) { if (result.ok) return '正在播报'; if (result.reason === 'deduplicated') return '已忽略重复播报'; if (result.reason === 'credentials-missing') return '请填写本机百度语音密钥'; if (result.reason === 'text-too-long') return '播报文字过长，请缩短后重试'; if (result.reason === 'baidu-failed') return '百度语音合成失败，请检查网络和密钥'; if (result.reason === 'playback-failed' || result.reason === 'playback-timeout') return '语音音频播放失败'; return '语音服务暂不可用'; }
function alarmControls(state, event) { const ack = state.alarmAckStatus || { key: '', stage: '' }; const guardianRole = state.guardian && state.guardian.role; const acknowledged = Boolean(event && event.acknowledgedBy && event.acknowledgedBy.length); const sameRequest = Boolean(event && ack.key === event.eventKey); const waiting = sameRequest && (ack.stage === 'sending' || ack.stage === 'waiting'); const failed = sameRequest && ack.stage === 'failed'; const deviceReady = Boolean(event && state.connected && state.deviceId === event.deviceId && guardianRole !== 'guardian'); return { acknowledgeDisabled: acknowledged, resolveDisabled: !deviceReady || waiting, resolveText: waiting ? (ack.stage === 'sending' ? '正在发送…' : '等待设备回执…') : failed ? '写入失败，点击重试' : deviceReady ? '请求设备解除' : '需连接当前设备', resolveStatus: waiting ? '等待设备返回匹配的 ACTIVE=0。' : failed ? '上次请求失败，可以重试。' : '' }; }
Page({
  data: { connected: false, statusTone: 'idle', statusLabel: '未连接', translation: '等待手势', recognition: '正在等待完整十指数据', ttsStatus: '外部语音服务待配置', left: [], right: [], pose: { roll: '--', pitch: '--', yaw: '--' }, lastFrame: '—', care: { hr: 0, spo2: 0, fall: false, sos: false }, safetyAlert: null, alarmTitle: '报警中心', alarmCopy: '查看真实 ALARM 事件；CARE 和姿态角不会冒充报警。', alarmAudioStatus: '本地报警音待触发', canResolveAlarm: false, guardianRole: 'unselected', acknowledgeDisabled: false, resolveDisabled: true, resolveText: '需连接当前设备', resolveStatus: '' },
  onLoad() { this.runtime = getApp().getRuntime(); this.store = createGestureStore(); this.gestures = this.store.list(); this.tts = createTtsService(); this.matcher = createStableMatcher({ holdMs: 350 }); this.lastSpoken = ''; this.active = false; this.displayPose = { roll: null, pitch: null, yaw: null }; this.targetPose = { roll: null, pitch: null, yaw: null }; this.lastRenderedPose = ''; this.lastSensorSignature = ''; this.lastStatusSignature = ''; this.lastCareSignature = ''; this.lastFrameAt = null; this.unsubscribe = this.runtime.subscribe((state) => this.applyState(state)); this.setData({ ttsStatus: this.tts.isConfigured() ? '百度语音服务已就绪' : '请填写本机百度语音密钥' }); },
  onShow() { this.active = true; this.gestures = this.store.list(); this.runtime.setMode(this.runtime.modes.translation); this.startPoseRenderer(); this.applyState(this.runtime.getState()); }, onHide() { this.active = false; this.stopPoseRenderer(); this.matcher.reset(); this.lastSpoken = ''; }, onUnload() { this.stopPoseRenderer(); if (this.unsubscribe) this.unsubscribe(); this.tts.stop(); },
