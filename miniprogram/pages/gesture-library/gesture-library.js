...
Page({
  ...
  resample(e) { const id = e.currentTarget.dataset.id; if (!this.store.list().some((item) => item.id === id)) return; wx.navigateTo({ url: `/pages/gesture-train/gesture-train?replaceId=${encodeURIComponent(id)}` }); },
